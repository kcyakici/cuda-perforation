...
__global__ void covar_kernel(int m, int n, DATA_TYPE *symmat, DATA_TYPE *data)
{
    ...
    if (j1 < _PB_M)
    {
        for (j2 = j1; j2 < _PB_M * LOOP_PERFORATION_RATE; j2++)
        {
            symmat[j1 * M + j2] = 0.0;
            for (i = 0; i < _PB_N * LOOP_PERFORATION_RATE; i++)
            {
                    ...
                    DATA_TYPE POLYBENCH_2D(symmat_outputFromGpu, M, M, m, m))
{
    ...
    dim3 grid2((size_t)(ceil((float)M) / ((float)DIM_THREAD_BLOCK_KERNEL_2_X) * GRID_PERFORATION_RATE), (size_t)(ceil((float)N) / ((float)DIM_THREAD_BLOCK_KERNEL_2_X) * GRID_PERFORATION_RATE));
    ...
    dim3 grid3((size_t)(ceil((float)M) / ((float)DIM_THREAD_BLOCK_KERNEL_3_X) * GRID_PERFORATION_RATE), 1);
