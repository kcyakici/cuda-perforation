                    ...
                    DATA_TYPE POLYBENCH_2D(X_fromGpu, N, N, n, n))
{
    ...
    for (i = 0; i < n; i++)
    {
        for (j = 0; j < n; j++)
        {
            ...
            {
                // printf("B: %lf, B_gpu: %lf\n", B_cpu[i][j], B_fromGpu[i][j]);
            }
        }
    }
...
__global__ void adi_kernel1(int n, DATA_TYPE *A, DATA_TYPE *B, DATA_TYPE *X)
{
    ...
    if ((i1 < _PB_N))
    {
        for (int i2 = 1; i2 < _PB_N * LOOP_PERFORATION_RATE; i2++)
        {
...
__global__ void adi_kernel3(int n, DATA_TYPE *A, DATA_TYPE *B, DATA_TYPE *X)
{
    ...
    if (i1 < _PB_N)
    {
        for (int i2 = 0; i2 < (_PB_N - 2) * LOOP_PERFORATION_RATE; i2++)
        {
             ...
             DATA_TYPE POLYBENCH_2D(B_outputFromGpu, N, N, n, n), DATA_TYPE POLYBENCH_2D(X_outputFromGpu, N, N, n, n))
{
    ...
    dim3 block1(DIM_THREAD_BLOCK_X * BLOCK_PERFORATION_RATE, DIM_THREAD_BLOCK_Y * BLOCK_PERFORATION_RATE, 1);
    dim3 grid1(1, 1, 1);
    grid1.x = (size_t)(ceil(((float)N) / ((float)block1.x) * GRID_PERFORATION_RATE));
    ...
    for (int t = 0; t < _PB_TSTEPS * KERNEL_LAUNCH_LOOP_RATE; t++)
    {
        ...
        for (int i1 = 1; i1 < _PB_N * KERNEL_LAUNCH_LOOP_RATE; i1++)
        {
        ...
        for (int i1 = 0; i1 < (_PB_N - 2) * KERNEL_LAUNCH_LOOP_RATE; i1++)
        {
