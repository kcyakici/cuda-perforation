              ...
              DATA_TYPE POLYBENCH_2D(ey, NX, NY, nx, ny), DATA_TYPE POLYBENCH_2D(hz, NX, NY, nx, ny), DATA_TYPE POLYBENCH_2D(hz_outputFromGpu, NX, NY, nx, ny))
{
    ...
    dim3 grid((size_t)ceil(((float)NY) / ((float)block.x) * GRID_PERFORATION_RATE), (size_t)ceil(((float)NX) / ((float)block.y) * GRID_PERFORATION_RATE));

    /* Start timer. */
    GpuTimer gpuTimer;
    gpuTimer.Start();

    for (int t = 0; t < _PB_TMAX * KERNEL_LAUNCH_LOOP_RATE; t++)
    {
