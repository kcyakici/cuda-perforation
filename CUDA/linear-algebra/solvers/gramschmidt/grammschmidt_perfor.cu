...
#define KERNEL_LAUNCH_LOOP_RATE 0.95
...
__global__ void gramschmidt_kernel1(int ni, int nj, DATA_TYPE *a, DATA_TYPE *r, DATA_TYPE *q, int k)
{
    ...
    if (tid == 0)
    {
        DATA_TYPE nrm = 0.0;
        int i;
        for (i = 0; i < _PB_NI * LOOP_PERFORATION_RATE; i++)
        {
...
void gramschmidtCuda(int ni, int nj, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj), DATA_TYPE POLYBENCH_2D(R, NJ, NJ, nj, nj), DATA_TYPE POLYBENCH_2D(Q, NI, NJ, ni, nj), DATA_TYPE POLYBENCH_2D(A_outputFromGpu, NI, NJ, ni, nj))
{
    ...
    dim3 gridKernel2((size_t)ceil(((float)NJ) / ((float)DIM_THREAD_BLOCK_X) * GRID_PERFORATION_RATE), 1);
    dim3 gridKernel3((size_t)ceil(((float)NJ) / ((float)DIM_THREAD_BLOCK_X) * GRID_PERFORATION_RATE), 1);
    ...
    int k;
    for (k = 0; k < _PB_NJ * KERNEL_LAUNCH_LOOP_RATE; k++)
    {
