...
__global__ void gemver_kernel2(int n, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE *a, DATA_TYPE *x, DATA_TYPE *y, DATA_TYPE *z, int perforated_pb_n)
{
...
__global__ void gemver_kernel3(int n, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE *a, DATA_TYPE *x, DATA_TYPE *w, int perforated_pb_n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
                ...
                DATA_TYPE POLYBENCH_1D(z, N, n))
{
    ...
    gemver_kernel2<<<grid2, block2>>>(n, alpha, beta, A_gpu, x_gpu, y_gpu, z_gpu, perforated_pb_n);
    cudaDeviceSynchronize();
    gemver_kernel3<<<grid3, block3>>>(n, alpha, beta, A_gpu, x_gpu, w_gpu, perforated_pb_n);
    cudaDeviceSynchronize();
