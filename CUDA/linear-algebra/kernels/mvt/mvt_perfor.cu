...
__global__ void mvt_kernel1(int n, DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1, int perforated_pb_n)
{
...
__global__ void mvt_kernel2(int n, DATA_TYPE *a, DATA_TYPE *x2, DATA_TYPE *y_2, int perforated_pb_n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
             ...
             DATA_TYPE POLYBENCH_1D(x1_outputFromGpu, N, n), DATA_TYPE POLYBENCH_1D(x2_outputFromGpu, N, n))
{
    ...
    float perforated_pb_n = _PB_N * LOOP_PERFORATION_RATE;
    ...
    dim3 grid((size_t)ceil((float)N / ((float)DIM_THREAD_BLOCK_X) * GRID_PERFORATION_RATE), 1);

    /* Start timer. */
    GpuTimer gpuTimer;
    gpuTimer.Start();

    mvt_kernel1<<<grid, block>>>(n, a_gpu, x1_gpu, y_1_gpu, perforated_pb_n);
    mvt_kernel2<<<grid, block>>>(n, a_gpu, x2_gpu, y_2_gpu, perforated_pb_n);
